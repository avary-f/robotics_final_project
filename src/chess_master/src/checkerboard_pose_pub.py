...
# Prepare object points (3D points of checkerboard corners in its own frame)
objp = np.zeros((CHECKERBOARD[0]*CHECKERBOARD[1], 3), np.float32)
objp[:, :2] = np.mgrid[0:CHECKERBOARD[0], 0:CHECKERBOARD[1]].T.reshape(-1, 2)
objp *= SQUARE_SIZE
...
def image_callback(msg):
    ...
    if ret:
        ...
        t.header.frame_id = "head_camera"
        t.child_frame_id = "checkerboard"
        t.transform.translation.x = float(tvec.flatten()[0])
        t.transform.translation.y = float(tvec.flatten()[1])
        t.transform.translation.z = float(tvec.flatten()[2])
        t.transform.rotation.x = quat[0]
        t.transform.rotation.y = quat[1]
        t.transform.rotation.z = quat[2]
        t.transform.rotation.w = quat[3]

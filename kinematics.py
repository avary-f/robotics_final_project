...
def dh2A(dh: list[float], jt: str) -> Callable[[float], NDArray]:
    ...
    if jt == 'r':
        ...
        def A(q: float) -> NDArray:
            ...
            alpha = dh[3]
            theta = theta + q
            # T = tr.rotz(theta)@ tr.transz(d)@tr.transx(a)@tr.rotx(alpha)
            T = np.array([
...
class SerialArm:
    ...
    def jacob(self, q: list[float]|NDArray, index: int|None=None, base: bool=False,
        ...
        # what is shown here.
        # point of interest im pretty sure is index, and also should be the 
        T_n_in_0 = self.fk(q, index, base, tip)
        # print("T_n_in_0:")
        # print(T_n_in_0)
        pe = T_n_in_0[0:3, 3]
        ...
        for i in range(index):
            ...
            if self.jt[i] == 'r':
                T_i_in_0 = self.fk(q, i, base, tip)
                Z_i_in_0 = T_i_in_0[0:3, 2]
                Z_i_in_0 = np.transpose(Z_i_in_0)
                o_i_in_0 = T_i_in_0[0:3, 3]
                J[0:3, i] = np.cross(Z_i_in_0, (pe -o_i_in_0))
                J[3:6, i] = Z_i_in_0
    ...
    def ik_position(self, target: NDArray, q0: list[float]|NDArray|None=None,
        ...
        def get_jacobian_transpose(q, J, e):
            qdot = J.T @ K @ e
            print(qdot)
            return qdot
    

        def get_damped_psuedo_inverse(q, J, e):
            kd = .01
            # dt = .1
        ...
        def get_error(q):
            cur_position = self.fk(q)[0:3, 3]       # this should get the location from transformation matrix from 0 to tip
            e = target - cur_position 
            return e


        iters = 0
        error = get_error(q)             # initialize error
        qdot = 0
        # viz = VizScene()
        # viz.add_marker(target)
        # # adding a SerialArm to the visualization, and telling it to draw the joint frames.
        # viz.add_arm(self, draw_frames=True)
        ...
        # it's moving in the right direction towards the target.
            ...
            # step 2: find qdotdes that moves arm in a good direction 
            J = self.jacob(q)
            J = J[0:3, :]
            if method == 'JT_T':
                # use the jacobian tranpose
                qdot = get_jacobian_transpose(q, J, error)
                # print("qdot output:", qdot)
            if method == 'pinv':
                # use the pinv transpose
                qdot = get_damped_psuedo_inverse(q, J, error)

            # step 3: take a small step in that direction (update q)
            iters += 1
            ...
            # viz.hold(.25)
            dt = .5
            q = q + qdot*dt            # is this the right way to update q? 

            # step 4: find error between T0des and T0n (where n is the tip of the robot)
            error = get_error(q)
